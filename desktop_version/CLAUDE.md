<!-- 작성: 2026-09-24 18:44 (수정: 2026-09-24 19:40 파일 연결 관계, 가중치 내보내기, 검증 데이터 추가) -->
...
- MNIST 테스트 이미지를 280x280(그림판 크기)으로 키워 `app.py`의 `전처리()` → 모델에 넣고, 그림·정답·전처리 결과·점수를 `web_version/검증데이터.json/.bin`으로 저장합니다.
- 크기가 크고(200장에 약 16MB) 언제든 다시 만들 수 있으므로 `.gitignore`로 저장소에서 뺍니다.
- 이 데이터로 `web_version/검증.html`이 웹 버전을 파이썬과 대조합니다. 앱 경로 자체의 정확도 측정값: 497/500 (이전), 200/200 (검증 데이터).
