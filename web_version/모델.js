// 작성: 2026-09-25 09:19 (수정: 2026-09-25 09:51 층 입력 형상 검사 추가)
  ...
  async function 모델_불러오기(기본주소 = "") {
    ...
    function 추론(입력) {
      ...
      for (const 층 of 층들) {
        if (층.종류 === "conv") {
          if (층.형상[1] !== 채널) {
            throw new Error(`합성곱 입력 채널이 맞지 않습니다: 기대 ${층.형상[1]}, 실제 ${채널}`);
          }
          ...
          if (층.형상[1] !== x.length) {
            throw new Error(`전결합 입력 크기가 맞지 않습니다: 기대 ${층.형상[1]}, 실제 ${x.length}`);
          }
